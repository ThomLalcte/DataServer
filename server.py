...
capLastSample = 0
lastBootTime = dt.now(pytz.timezone('Canada/Eastern'))
wake: int = 0
...
def processData(date: dt, dataType: str):
    ...
    for ii in range(-4, min(date.hour+1, 14)):  # on itère les heures demandées
        # contien la journée corrigé pour les heures négative (-1 jour)
        day = (date-dtd(ii < 0)).strftime("%Y-%m-%d")
        # si le jour ou l'heure est pas listé on envoie des -1
        if not day in filedata.keys() or not str(ii) in filedata[day].keys():
            for iii in range(20):
                data.append(-1)
                time.append((date-dtd(ii < 0)).replace(hour=ii +
                            24*(ii < 0), minute=iii*3))
            continue
        # contient heure corrigée pour heures négatives
        hour = str(ii+24*(ii < 0))
    ...
    tts = (time[deriv.index(maxDeriv)] -
           time[deriv.index(minDeriv)])  # total time slept
...
def provideData(client: socket.socket):
    ...
    print("{}, {}, {} jours de {}h à {}h".format(
        datatype, date, dateDelta, hourmin, hourend))
    ...
    for i in range(dateDelta):  # on itère les dates demandeés
        for ii in range(hourmin, hourend):  # on itère les heures demandées
            # contien la journée corrigé pour les heures négative (-1 jour)
            day = (datedt-dtd(i+(ii < 0))).strftime("%Y-%m-%d")
            # si le jour ou l'heure est pas listé on envoie des -1
            if not day in filedata.keys() or not str(ii+24*(ii < 0)) in filedata[day].keys():
                for iii in range(20):
            ...
            # envoi de -1 pour ceux qui exisntent pas
            for iii in range(20-len(filedata[day][hour])):
                client.send(int.to_bytes(-1, 4, "big", signed=True))
    client.close()
...
def handleScaleClient(client: socket.socket):
    # send ok to client with null terminated string
    import time
    client.send(b"ok")
    samplePeriod = int.from_bytes(client.recv(2), "little", signed=False)
    qte = int.from_bytes(client.recv(2), "little", signed=False)
    battery = int.from_bytes(client.recv(2), "little", signed=False)
    # print(f"sample period: {samplePeriod/1000} seconds")
    # print(f"sample quantity: {qte}")
    print(f"scale reports {qte} samples over {qte*samplePeriod/1000} seconds")
    # save data to buffer until client closes connection
    buffer = []
    for sample in range(qte):
        # buffer.append(client.recv(4))
        buffer.append(int.from_bytes(client.recv(4), "little", signed=True))
        time.sleep(samplePeriod/950)
    client.close()
    # print(buffer)
    # save buffer to csv file with timestamp
    with open(f"{dt.now(pytz.timezone('Canada/Eastern')).strftime('data/scale/%Y-%m-%d_%H-%M-%S')}.csv", "w") as file:
        file.write(f"{samplePeriod}\n")
        file.write(f"{battery}\n")
        for sample in range(qte):
            file.write(f"{buffer[sample]}\n")
            # file.write(f"{buffer[sample*4:sample*4+4]}\n")
        file.close()
...
while True:
    try:
        ...
        elif content == b"scale":
            scaleh = threading.Thread(target=handleScaleClient, args=(client,))
            scaleh.start()
...
if errors > 0:
    with open("data/server_log_data.json", "r") as file:
        data = js.load(file)
        file.close()
    today = str(dt.now().year) + "-" + \
        str(dt.now().month) + "-" + str(dt.now().day)
